...
if __name__ == '__main__':
    ...
    out_path=os.path.join('out',models,'area',ts)
    ...
    for i,f in enumerate(os.listdir(in_path)):
        ...
        selection = r.point(lambda i: 0 if i > 120 else 250)
        selection.save(os.path.join(out_path,'mask_'+f))
        r.paste(g, None, selection)
        img2 = Image.merge( "RGBA", (r, g, b, alpha))
        img2.save( out_img)
        #img.show()
        #img2.show()
        #cv2.waitKey(1)
