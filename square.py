...
if __name__ == '__main__':
    ...
    out_path=os.path.join('out',models,'square',ts)
    ...
    for i,f in enumerate(os.listdir(in_path)):
        ...
        x = float(height if height > width else width)
        y = float(height if height > width else width)
        square = np.zeros((x,y,3), np.uint8)
        print((x-width)/2,x-(x-width)/2, (x-width)/2 - (x-(x-width)/2))  
